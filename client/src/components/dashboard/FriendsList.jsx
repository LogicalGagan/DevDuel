      ...
      <div className="px-5 py-3.5 border-b border-[var(--lc-border)] flex items-center gap-3 flex-wrap">
        ...
        <button
          ...
          className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold transition-all ${
            panel === 'requests'
              ? 'border-[var(--lc-accent-orange)]/50 bg-orange-900/20 text-orange-300'
              : 'border-[var(--lc-border)] text-[var(--lc-text-primary)] hover:border-orange-400/50 hover:text-orange-300'
          }`}
        >
          🔔 Requests
          {requests.length > 0 && (
            <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-[var(--lc-accent-orange)] text-[9px] font-black text-white flex items-center justify-center">
              {requests.length}
            </span>
          )}
        </button>
...
const RequestsPanel = ({ requests, busy, onAccept, onDecline }) => (
  <div className="space-y-3">
    <p className="text-xs font-bold text-[var(--lc-text-bright)]">
      Friend Requests
      {requests.length > 0 && (
        <span className="ml-2 px-1.5 py-0.5 rounded-full bg-[var(--lc-accent-orange)]/20 border border-[var(--lc-accent-orange)]/40 text-orange-300 text-[10px]">
          {requests.length}
        </span>
      )}
    </p>
    ...
    {requests.length === 0 ? (
      ...
      <div className="space-y-2 max-h-64 overflow-y-auto">
        {requests.map((req) => {
          ...
          return (
            <div key={fromId} className="rounded-xl border border-[var(--lc-accent-orange)]/20 bg-orange-900/5 p-3 space-y-2">
