...
const Signup = () => {
  ...
  const inputClasses = "w-full bg-slate-900/60 border border-slate-600/50 rounded-xl px-4 py-3 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-[var(--lc-accent-orange)]/60 focus:ring-1 focus:ring-violet-500/30 transition-all";
  ...
  const getStrength = () => {
    ...
    if (score <= 1) return { width: '40%', color: 'bg-[var(--lc-accent-orange)]', label: 'Fair' };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-1/3 right-1/3 w-96 h-96 bg-[var(--lc-accent-orange)]/5 rounded-full blur-3xl" />
      </div>
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          <Link to="/" className="inline-block">
            <div className="flex items-center justify-center gap-2">
              <SwordsIcon size={36} className="text-[var(--lc-accent-orange)]" />
              <span className="bg-gradient-to-r from-violet-400 via-violet-500 to-indigo-500 bg-clip-text text-transparent">DevDuel</span>
            </div>
          </Link>
        ...
        <div className="rounded-2xl p-8 bg-slate-800/40 border border-slate-700/50 backdrop-blur shadow-2xl shadow-black/20">
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              ...
              <p className="text-center text-sm text-slate-400">
                ...
                <button type="button" onClick={handleResendOtp} disabled={resendLoading}
                  className="font-bold text-[var(--lc-accent-orange)] hover:text-orange-300 transition-colors disabled:opacity-50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex bg-slate-900/60 p-1 rounded-xl border border-slate-600/50 mb-6">
              <button
                ...
                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${role === 'user' ? 'bg-[var(--lc-accent-orange)] text-slate-900 shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
              >
                User
              </button>
              <button
                ...
                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${role === 'admin' ? 'bg-[var(--lc-accent-orange)] text-slate-900 shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
              >
                Admin
              </button>
            </div>
          ...
          <p className="mt-6 text-center text-sm text-slate-400">
            ...
            <Link to="/login" className="font-bold text-[var(--lc-accent-orange)] hover:text-orange-300 transition-colors">
              Sign in
            </Link>
          </p>
        </div>
