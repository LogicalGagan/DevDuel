      ...
      {submissionStatus && (
        ...
        >
          <div className="flex gap-1 items-center">
            <div className="w-6 h-[2px] rounded-full bg-[var(--lc-border)] group-hover:bg-[var(--lc-accent-orange)] transition-colors" />
            <div className="w-1.5 h-1.5 rounded-full bg-[var(--lc-border)] group-hover:bg-[var(--lc-accent-orange)] transition-colors" />
            <div className="w-6 h-[2px] rounded-full bg-[var(--lc-border)] group-hover:bg-[var(--lc-accent-orange)] transition-colors" />
          </div>
        </div>
      )}
          ...
          >
            ...
            {isCE && submissionStatus.compilationError && (
              <div className="px-5 py-3 shrink-0" style={{ borderBottom: '1px solid var(--lc-border)' }}>
                <p className="text-[10px] font-mono font-bold text-[var(--lc-accent-orange)] uppercase tracking-widest mb-2">Compiler Output</p>
              ...
              <div className="flex-1 flex items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                  <div className="relative w-10 h-10">
                    <div className="absolute inset-0 rounded-full border-2 border-[var(--lc-accent-orange)]/20 animate-ping" />
                    <div className="w-10 h-10 rounded-full border-2 border-t-transparent border-[var(--lc-accent-orange)] animate-spin" />
                  </div>
                    ...
                    {!test.passed && !isCE && (
                      <div className="px-4 py-3 space-y-2">
                        <div className="flex items-start gap-2 text-xs font-mono">
                          <span className="text-[var(--lc-accent-blue)] font-bold shrink-0 w-16">Expected</span>
                        ...
                        {test.stderr && (
                          <div className="flex items-start gap-2 text-xs font-mono">
                            <span className="text-[var(--lc-accent-orange)] font-bold shrink-0 w-16">Stderr</span>
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored.currentProblem && currentRound > 0) {
        setActiveProblem(stored.currentProblem);
        setCode('// Write your code here\n'); setOpponentStatus('idle'); addEvent('Round ' + (currentRound + 1) + ' started.', 'var(--lc-accent-orange)');
      }
    } catch { }
  }, [currentRound]);
      ...
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--lc-bg)' }}>
        <div className="flex flex-col items-center gap-5">
          <div className="relative w-14 h-14">
            <div className="absolute inset-0 rounded-full border-2 border-[var(--lc-accent-orange)]/20 animate-ping" />
            <div className="w-14 h-14 rounded-full border-2 border-t-transparent border-[var(--lc-accent-orange)] animate-spin" />
          </div>
          <p className="font-mono text-sm tracking-widest uppercase" style={{ color: 'var(--lc-accent-orange)' }}>Loading Arena...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{ backgroundColor: 'var(--lc-bg)', color: 'var(--lc-text-primary)' }}>
        ...
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex items-center gap-2 shrink-0">
            <div className="w-7 h-7 rounded-md flex items-center justify-center font-black text-xs text-black"
              style={{ background: 'linear-gradient(135deg,var(--lc-accent-orange),#6366f1)' }}>CC</div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg"
            ...
            <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0"
              style={{ background: 'linear-gradient(135deg,var(--lc-accent-orange),#6366f1)' }}>
          ...
          {user?.picture
            ...
            : <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white"
              style={{ background: 'linear-gradient(135deg,var(--lc-accent-orange),#6366f1)' }}>{user?.name?.[0] || 'Y'}</div>
          }
        </div>
      </header>
      ...
      {roundWonResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)' }}>
          <div className="rounded-2xl p-8 text-center w-80 shadow-2xl"
            style={{ backgroundColor: 'var(--lc-nav)', border: '1px solid var(--lc-border)', boxShadow: '0 0 60px rgba(139,92,246,0.12)' }}>
            <div className="flex justify-center mb-4">{roundWonResult.winnerId === userId ? <TrophyIcon size={64} className="text-[var(--lc-accent-orange)]" /> : <FlameIcon size={64} className="text-red-500 opacity-60" />}</div>
            ...
            {currentRound < displayTotalRounds - 1 && (
              <p className="mt-4 text-xs font-mono animate-pulse" style={{ color: 'var(--lc-accent-orange)' }}>⟳ Next round loading...</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
          ...
          <div className="flex shrink-0" style={{ borderBottom: '1px solid var(--lc-border)' }}>
            {['Description', 'Solutions', 'Submissions'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                ...
                style={{
                  color: activeTab === tab ? 'var(--lc-accent-orange)' : '#6b7280',
                  borderBottom: activeTab === tab ? '2px solid var(--lc-accent-orange)' : '2px solid transparent',
                  ...
                  <ul className="space-y-1.5">
                    {[
                      ...
                      <li key={i} className="flex items-start gap-2 text-xs font-mono text-gray-400">
                        <span style={{ color: 'var(--lc-accent-orange)' }}>•</span>{c}
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        >
          <div className="w-[2px] h-8 rounded-full bg-[var(--lc-border)] group-hover:bg-[var(--lc-accent-orange)] transition-colors duration-150" />
        </div>
            ...
            <div className="flex items-center gap-0.5 rounded-lg p-0.5"
              ...
              {[{ id: 71, s: 'Python 3' }, { id: 63, s: 'JavaScript' }, { id: 62, s: 'Java' }, { id: 54, s: 'C++' }].map(l => (
                <button key={l.id} onClick={() => setLanguage(l.id)}
                  ...
                  style={language === l.id
                    ? { background: 'linear-gradient(135deg,var(--lc-accent-orange),#6366f1)', color: '#1a1a1a' }
          ...
          <div className="flex items-center justify-between px-3 py-2 shrink-0"
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => handleSubmit(true)}
                ...
                style={{ background: 'linear-gradient(135deg,var(--lc-accent-orange),#6366f1)', color: '#1a1a1a', boxShadow: '0 0 12px rgba(139,92,246,0.25)' }}>
            ...
            <div className="flex items-center gap-3 p-3 rounded-xl"
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] font-mono mt-0.5"
                  style={{
                    color: opponentStatus === 'typing' ? 'var(--lc-accent-orange)'
