...
const CreateContest = () => {
  ...
  const inputClasses = "w-full bg-slate-900/60 border border-slate-600/50 rounded-xl px-4 py-2.5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-[var(--lc-accent-orange)]/60 focus:ring-1 focus:ring-violet-500/30 transition-all font-mono";
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 pt-4 pb-16">
        <div className="max-w-4xl mx-auto px-5 py-4">
          ...
          <button onClick={() => navigate('/contests')}
            className="text-sm font-mono text-slate-500 hover:text-[var(--lc-accent-orange)] mb-4 transition-colors inline-flex items-center gap-1">
            ← Back to Contests
          </button>

          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-black mb-2 bg-gradient-to-r from-violet-400 via-violet-500 to-indigo-500 bg-clip-text text-transparent">
              ✨ Create Contest
            </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="rounded-2xl p-6 space-y-4 bg-slate-800/40 border border-slate-700/50 backdrop-blur shadow-lg">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-9 h-9 rounded-lg bg-[var(--lc-accent-orange)]/20 border border-orange-400/30 flex items-center justify-center text-lg">📝</div>
                <div>
              ...
              <div className="flex items-center justify-between mb-5 border-b pb-4" style={{ borderColor: 'var(--lc-border)' }}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="px-3 py-1 rounded-lg text-xs font-black font-mono bg-slate-900/60 border border-slate-700/50"
                    style={{ color: form.selectedProblems.length > 0 ? 'var(--lc-accent-orange)' : '#64748b' }}>
              ...
              {form.selectedProblems.length > 0 && (
                <div className="mb-5 space-y-1.5">
                  ...
                  {form.selectedProblems.map((pid, idx) => {
                    ...
                    return (
                      <div key={pid} className="flex items-center gap-2 p-3 rounded-xl text-sm bg-slate-900/60 border border-[var(--lc-accent-orange)]/20 shadow-sm">
                  ...
                  {problems.filter((p) => !form.selectedProblems.includes(p.id)).map((p) => {
                    ...
                    return (
                      <button key={p.id} type="button" onClick={() => toggleProblem(p.id)}
                        className="w-full flex items-center gap-3 p-3 rounded-xl text-sm text-left transition-all border bg-slate-900/30 border-slate-700/40 hover:border-[var(--lc-accent-orange)]/30 hover:bg-slate-800/50 group">
            ...
            <button type="submit" disabled={creating}
              ...
              style={{ background: 'linear-gradient(135deg,var(--lc-accent-orange),#6366f1)', color: '#1a1a1a' }}>
