        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg"
            ...
            <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0"
              style={{ background: timeLeft === 'Ended' ? 'linear-gradient(135deg,#ef4444,#dc2626)' : 'linear-gradient(135deg,var(--lc-accent-orange),#6366f1)' }}>
            ...
            <span className="font-mono text-sm font-black tabular-nums"
              style={{ color: timeLeft === 'Ended' ? '#ef4444' : 'var(--lc-accent-orange)' }}>
              {timeLeft || '--:--'}
            </span>
          </div>
          ...
          {user?.picture
            ...
            : <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white"
              style={{ background: 'linear-gradient(135deg,var(--lc-accent-orange),#6366f1)' }}>{user?.name?.[0] || 'U'}</div>
          }
        </div>
      </header>
            ...
            {contest.problems?.map((cp, idx) => {
              ...
              return (
                <button key={idx} onClick={() => { setSelectedProblem(idx); setResult(null); setActiveTab('Description'); }}
                  ...
                  style={{
                    color: isActive ? 'var(--lc-accent-orange)' : solved ? '#00b8a3' : '#6b7280',
                    borderBottom: isActive ? '2px solid var(--lc-accent-orange)' : '2px solid transparent',
          ...
          <div className="overflow-y-auto shrink-0" style={{ maxHeight: '240px', borderBottom: '1px solid var(--lc-border)' }}>
            {leaderboard.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 gap-2">
                <TrophyIcon size={32} className="text-[var(--lc-accent-orange)]" />
                ...
                {leaderboard.map((entry, idx) => {
                  ...
                  return (
                    <div key={entry.userId || idx}
                      ...
                      style={{ backgroundColor: isMe ? 'rgba(139,92,246,0.08)' : 'transparent' }}>
                      <span className="w-6 text-center font-black font-mono"
                        style={{ color: idx < 3 ? 'var(--lc-accent-orange)' : '#6b7280' }}>
                        {medal || (idx + 1)}
                      </span>
                      <div className="flex-1 min-w-0">
                        <span className="text-xs font-mono font-bold truncate block"
                          style={{ color: isMe ? 'var(--lc-accent-orange)' : '#fff' }}>
