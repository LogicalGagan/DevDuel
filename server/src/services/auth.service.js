...
function sendOtpEmail(toEmail, otp, subject = 'Your DevDuel verification code') {
  const transporter = getEmailTransporter();
  if (!transporter) {
    console.log(`\n📬 [DEV] SMTP not configured. Verification code for ${toEmail}: ${otp}\n`);
    return;
  }
  return transporter.sendMail({
    from: `"DevDuel" <${process.env.SMTP_USER}>`,
    to: toEmail,
    subject,
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:400px;margin:0 auto;">
        <h2 style="color:#ff7a00;">DevDuel</h2>
