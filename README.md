...
DevDuel is a high-performance, real-time competitive programming platform designed to host large-scale contests and 1v1 coding battles. Built with a focus on low latency and high concurrency, it features an isolated code execution engine and real-time synchronization across hundreds of users.
...
- **⚙️ Secure Code Execution**: Multi-language support (Python, C++, Java, etc.) using Dockerized local Judge0 for safe and fast grading.
...
- **📊 Performance Optimized**:
  ...
  - **Worker Pattern**: Code execution is handled by separate `dev-duel-worker` processes via BullMQ to keep the main API responsive.
...
- **Execution Engine**: Judge0 (running inside isolated Docker containers).
- **Process Management**: PM2 (API + Worker split).

## 📊 Scale & Performance

DevDuel has been battle-tested with a 500-user concurrent load test on an **AWS t3.small** instance:
...
1. **Clone the repository**:
   ```bash
   git clone https://github.com/rajats496/code-clash.git dev-duel
   cd dev-duel
   ```
...
```bash
# Start API
pm2 start src/server.js --name dev-duel-api
# Start Worker
pm2 start worker.js --name dev-duel-worker
```
